...
cone_combinations(Top_flavors, Bottom_flavors) 
  when Top_flavors == nil-> fail;
  cone_combinations([], Bottom_flavors) -> [];
  cone_combinations(Top_flavors, []) -> [];
  cone_combinations(Top_flavors, Bottom_flavors) ->
	[{X, Y} || X <- Top_flavors, Y <- Bottom_flavors].

% TODO: Complete this function so it passes all of its unit tests
cone_combinations(Flavor_list) when Flavor_list == nil -> fail;
cone_combinations(Flavor_list) when Flavor_list == [] -> [];
cone_combinations(Flavor_list) -> 
	[{X, Y} || X <- Flavor_list, Y <- Flavor_list].

% TODO: Complete this function so it passes all of its unit tests
most_popular_combinations(Count,List) when Count < 0 -> fail;
...
most_popular_combinations_test_()->
	...
	[?_assertEqual(H, most_popular_combinations(10, Sorted_combinations_with_purchases)),% Happy path
	 % Less obvious things we need to test start here. Let's test the edge cases.
