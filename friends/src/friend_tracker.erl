...
has_friends(Friends, Friend_list)->
	lists:all(fun(Friend)-> lists:member(Friend, Friend_list) end, Friends).

remove_friends(Friends, Friend_list)->
	lists:foldl(fun(Friend, Acc)-> lists:delete(Friend, Acc) end, Friend_list, Friends).
...
run(Friend_list) ->
	receive
		{From, {add, Friend}} ->
			New_list = add_friend(Friend, Friend_list),
			From ! received,
			run(New_list);
		{From, {add, Friends}} ->
			New_list = add_friends(Friends, Friend_list),
			From ! received,
			run(New_list);
		{From, {has_friend, Friend}} ->
			From ! has_friend(Friend, Friend_list),
			run(Friend_list);
		{From, {has_friends, Friends}} ->
			From ! has_friends(Friends, Friend_list),
			run(Friend_list);
		{From, {remove, Friend}} ->
			New_list = remove_friend(Friend, Friend_list),
			From ! received,
			run(New_list);
		{From, {remove, Friends}} ->
			New_list = remove_friends(Friends, Friend_list),
			From ! received,
			run(New_list);
		{From, get} ->
			From ! Friend_list,
